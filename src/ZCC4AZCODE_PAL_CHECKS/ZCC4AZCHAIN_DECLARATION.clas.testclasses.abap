class test definition final for testing
  ...
  private section.
    constants test_class type c length 30 value 'ZCC4AZTEST_CHAIN_DECLARATION'.
    constants:
...
class test implementation.
  ...
  method execute_test_class.
    ...
    cl_ci_atc_unit_driver=>create_asserter( )->check_and_assert(
              check             = new ZCC4AZCHAIN_DECLARATION( )
              object            = value #( type = 'CLAS' name = test_class )
              expected_findings = value #(
                code = ZCC4AZCHAIN_DECLARATION=>finding_codes-chain_declaration
                ( location = section_finding_1
                    quickfixes = value #( (
                    quickfix_code = ZCC4AZCHAIN_DECLARATION=>quickfix_codes-resolve_chain
