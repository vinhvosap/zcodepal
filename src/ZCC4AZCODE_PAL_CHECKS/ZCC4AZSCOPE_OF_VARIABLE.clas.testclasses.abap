CLASS test DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    CONSTANTS test_class TYPE c LENGTH 30 VALUE 'ZCC4AZTEST_SCOPE_OF_VARIABLE'.
    METHODS test FOR TESTING RAISING cx_static_check.
ENDCLASS.

CLASS ZCC4AZSCOPE_OF_VARIABLE DEFINITION LOCAL FRIENDS test.
...
CLASS test IMPLEMENTATION.
  ...
  METHOD test.
    ...
    cl_ci_atc_unit_driver=>create_test_case(
      check = NEW ZCC4AZSCOPE_OF_VARIABLE( )
      object = VALUE #( type = 'CLAS' name = test_class )
    )->execute_and_assert( VALUE #(
     ( code = ZCC4AZSCOPE_OF_VARIABLE=>message_codes-scope
       location = VALUE #( object = test1 position = VALUE #( line = 4 column = 6 ) )
       quickfixes = VALUE #( (
         quickfix_code = ZCC4AZSCOPE_OF_VARIABLE=>quickfix_codes-change_scope
