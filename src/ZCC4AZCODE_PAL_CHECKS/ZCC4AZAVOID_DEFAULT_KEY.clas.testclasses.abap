class test definition final for testing
  ...
  private section.
    constants test_class type c length 30 value 'ZCC4AZTEST_AVOID_DEFAULT_KEY'.
    constants:
...
class test implementation.
  ...
  method execute_test_class.
    ...
    cl_ci_atc_unit_driver=>create_asserter( )->check_and_assert(
      check             = new ZCC4AZAVOID_DEFAULT_KEY( )
      object            = value #( type = 'CLAS' name = test_class )
      expected_findings = value #(
        ( code = ZCC4AZAVOID_DEFAULT_KEY=>finding_codes-default_key
          location = psuedo_comment_1
          quickfixes = value #( (
            quickfix_code = ZCC4AZAVOID_DEFAULT_KEY=>quickfix_codes-empty_key
            location = psuedo_comment_1
            code = value #(
