...
class test definition final
  ...
  private section.
    constants test_class type c length 30 value 'ZCC4AZTEST_PROPER_BOOL_EXPR'.
    constants:
...
class test implementation.
  method execute_test_class.
    data(transform_to_xsd_findings) = value if_ci_atc_unit_asserter=>ty_expected_findings(
      code = ZCC4AZPROPER_BOOL_EXPRESSION=>finding_codes-transform_to_xsd
        ( location = if_then_else( value #( line = 5 column = 4 ) )
           quickfixes = value #( (
           quickfix_code = ZCC4AZPROPER_BOOL_EXPRESSION=>quickfix_codes-if_else
           location = if_then_else( value #( line = 5 column = 4 ) )
           code = value #(
    ...
    data(bool_value_findings) = value if_ci_atc_unit_asserter=>ty_expected_findings(
      code = ZCC4AZPROPER_BOOL_EXPRESSION=>finding_codes-boolean_value
      ( location = bool_usage( value #( line = 3 column = 4 ) )
          quickfixes = value #( (
          quickfix_code = ZCC4AZPROPER_BOOL_EXPRESSION=>quickfix_codes-character_equivalence
          location = bool_usage( value #( line = 3 column = 4 ) )
          code = value #(
            ( `T = ABAP_TRUE .` ) ) ) ) )
    ...
    data(initial_findings) = value if_ci_atc_unit_asserter=>ty_expected_findings(
      code = ZCC4AZPROPER_BOOL_EXPRESSION=>finding_codes-initial
      ( location = bool_initial( value #( line = 4 column = 4 ) )
          quickfixes = value #( (
          quickfix_code = ZCC4AZPROPER_BOOL_EXPRESSION=>quickfix_codes-initial_boolean
          location = bool_initial( value #( line = 4 column = 4 ) )
          code = value #(
    ...
    cl_ci_atc_unit_driver=>create_asserter( )->check_and_assert(
              check             = new ZCC4AZPROPER_BOOL_EXPRESSION( )
