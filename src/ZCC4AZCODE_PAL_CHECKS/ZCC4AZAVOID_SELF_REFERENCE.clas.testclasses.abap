class test definition final for testing
  ...
  private section.
    constants test_class type c length 30 value 'ZCC4AZTEST_AVOID_SELF_REF'.
    constants:
...
class test implementation.
  method execute_test_class.
    ...
    data(local_finding_1) = value if_ci_atc_check=>ty_location(
        object   = value #( type = 'PROG' name = 'ZCC4AZTEST_AVOID_SELF_REF=====CCIMP' )
    ...
    data(test_class_finding_1) = value if_ci_atc_check=>ty_location(
        object   = value #( type = 'PROG' name = 'ZCC4AZTEST_AVOID_SELF_REF=====CCAU' )
    ...
    cl_ci_atc_unit_driver=>create_asserter( )->check_and_assert(
          check             = new ZCC4AZAVOID_SELF_REFERENCE( )
          object            = value #( type = 'CLASS' name = test_class )
          expected_findings = value #(
            ( code = ZCC4AZAVOID_SELF_REFERENCE=>finding_codes-self_reference
                location = with_pseudo_comment_1
                quickfixes = value #( (
                quickfix_code = ZCC4AZAVOID_SELF_REFERENCE=>quickfix_codes-self_reference
                location = with_pseudo_comment_1
                code = value #(
