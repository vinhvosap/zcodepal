class ZCC4AZABAP_ANALYZER definition
  public
  final
  create private.

  public section.
    interfaces ZCC4AZIF_ABAP_ANALYZER.

    class-methods create returning value(instance) type ref to ZCC4AZIF_ABAP_ANALYZER.
    class-methods class_constructor.
    aliases find_clause_index for ZCC4AZIF_ABAP_ANALYZER~find_clause_index.
    aliases is_token_keyword for ZCC4AZIF_ABAP_ANALYZER~is_token_keyword.
    aliases is_db_statement for ZCC4AZIF_ABAP_ANALYZER~is_db_statement.
    aliases is_bracket for ZCC4AZIF_ABAP_ANALYZER~is_bracket.
    aliases max_line_length for ZCC4AZIF_ABAP_ANALYZER~max_line_length.
  ...
  private section.
    ...
    types:
      begin of ty_logical_expression_node,
        connective type ZCC4AZIF_ABAP_ANALYZER=>ty_logical_connective,
        left type i,
        right type i,
    ...
    methods _break_into_lines
      ...
      raising   ZCC4AZCX_LINE_BREAK_IMPOSSIBLE.
...
CLASS ZCC4AZABAP_ANALYZER IMPLEMENTATION.


  method ZCC4AZIF_ABAP_ANALYZER~break_into_lines.
    code_lines = _break_into_lines( code = code break_at = max_line_length ).
  endmethod.


  method ZCC4AZIF_ABAP_ANALYZER~calculate_bracket_end.
    if is_bracket( statement-tokens[ bracket_position ] ) ne ZCC4AZIF_ABAP_ANALYZER=>bracket_type-opening and
       is_bracket( statement-tokens[ bracket_position ] ) ne ZCC4AZIF_ABAP_ANALYZER=>bracket_type-closing.
      raise exception new ZCC4AZCX_TOKEN_IS_NO_BRACKET( ).
    endif.
    ...
    loop at statement-tokens assigning field-symbol(<token>) from bracket_position + 1.
      ...
      case is_bracket( <token> ).
        when ZCC4AZIF_ABAP_ANALYZER=>bracket_type-opening.
          bracket_counter += 1.

        when ZCC4AZIF_ABAP_ANALYZER=>bracket_type-closing.
        ...
        when ZCC4AZIF_ABAP_ANALYZER=>bracket_type-clopening.
          if bracket_counter eq 1.
            end_of_bracket = idx.
            exit.
          endif.
  ...
  method ZCC4AZIF_ABAP_ANALYZER~find_key_words.
    position = -1.
  ...
  method ZCC4AZIF_ABAP_ANALYZER~is_bracket.
    ...
    bracket_type = switch #(
      last_char
        when ')' then ZCC4AZIF_ABAP_ANALYZER=>bracket_type-closing
        when '(' then switch #(
          first_char
            when ')' then ZCC4AZIF_ABAP_ANALYZER=>bracket_type-clopening
            else ZCC4AZIF_ABAP_ANALYZER=>bracket_type-opening )
        else ZCC4AZIF_ABAP_ANALYZER=>bracket_type-no_bracket ).
  endmethod.


  method ZCC4AZIF_ABAP_ANALYZER~is_logical_connective.
    if token-references is initial.
      return switch #( token-lexeme
        when 'AND' then ZCC4AZIF_ABAP_ANALYZER=>logical_connective-and
        when 'OR' then ZCC4AZIF_ABAP_ANALYZER=>logical_connective-or
        when 'EQUIV' then ZCC4AZIF_ABAP_ANALYZER=>logical_connective-equiv
        when 'NOT' then ZCC4AZIF_ABAP_ANALYZER=>logical_connective-not ).
    else.
      return ZCC4AZIF_ABAP_ANALYZER=>logical_connective-none.
    endif.
  endmethod.


  method ZCC4AZIF_ABAP_ANALYZER~is_token_keyword.
    result = abap_true.
  ...
  method ZCC4AZIF_ABAP_ANALYZER~negate_comparison_operator.
    if not ZCC4AZIF_ABAP_ANALYZER~token_is_comparison_operator( value #( lexeme = comparison_operator ) ).
      raise exception new ZCC4AZCX_TOKEN_IS_NO_OPERATOR( ).
    endif.
    negated_comparison_operator = negations[ operator = comparison_operator ]-negated.
  endmethod.


  method ZCC4AZIF_ABAP_ANALYZER~parse_method_definition.
    if statement-keyword <> 'METHODS' and statement-keyword <> 'CLASS-METHODS'.
      return.
    endif.
    data(current_kind) = ZCC4AZIF_ABAP_ANALYZER=>parameter_kind-importing.
    ...
    loop at statement-tokens assigning field-symbol(<token>).
      ...
      if <token>-references is initial.
        case <token>-lexeme.
          when 'IMPORTING'.
            current_kind = ZCC4AZIF_ABAP_ANALYZER=>parameter_kind-importing.
          when 'EXPORTING'.
            current_kind = ZCC4AZIF_ABAP_ANALYZER=>parameter_kind-exporting.
          when 'CHANGING'.
            current_kind = ZCC4AZIF_ABAP_ANALYZER=>parameter_kind-changing.
          when 'RETURNING'.
            current_kind = ZCC4AZIF_ABAP_ANALYZER=>parameter_kind-returning.
          when 'TYPE'.
  ...
  method ZCC4AZIF_ABAP_ANALYZER~token_is_comparison_operator.
    case token-lexeme.
  ...
  method create.
    instance = new ZCC4AZABAP_ANALYZER( ).
  endmethod.


  method ZCC4AZIF_ABAP_ANALYZER~is_db_statement.
  ...
  method _break_into_lines.
    ...
    while i < strlen( code ).
      ...
      if i > break_at.
        if last_space = 0.
          raise exception type ZCC4AZCX_LINE_BREAK_IMPOSSIBLE.
        else.
  ...
  method ZCC4AZIF_ABAP_ANALYZER~negate_logical_expression.
    ...
    elseif lines( expression ) > 1.
      ...
      case <connective>-connective.
        when ZCC4AZIF_ABAP_ANALYZER=>logical_connective-and or ZCC4AZIF_ABAP_ANALYZER=>logical_connective-or.
          data(left) = ZCC4AZIF_ABAP_ANALYZER~negate_logical_expression(
            value #( for <t> in new_tokens from expression[ 2 ]-tokens-from to expression[ 2 ]-tokens-to ( <t> ) ) ).
          data(right) = ZCC4AZIF_ABAP_ANALYZER~negate_logical_expression(
            value #( for <t> in new_tokens from expression[ 3 ]-tokens-from to expression[ 3 ]-tokens-to ( <t> ) ) ).
        case <connective>-connective.
          when ZCC4AZIF_ABAP_ANALYZER=>logical_connective-and.
            return |{ left } OR { right }|.
          when ZCC4AZIF_ABAP_ANALYZER=>logical_connective-or.
      ...
      elseif is_token_keyword( token = new_tokens[ tokens_end - 1 ] keyword = 'NOT' ).
        delete new_tokens index tokens_end - 1.
      elseif ZCC4AZIF_ABAP_ANALYZER~token_is_comparison_operator( token = new_tokens[ tokens_end - 1 ] ).
        if new_tokens[ tokens_end ]-lexeme = 'ABAP_TRUE' or new_tokens[ tokens_end ]-lexeme = 'ABAP_FALSE'.
          ...
          new_tokens[ tokens_end - 1 ]-lexeme = ZCC4AZIF_ABAP_ANALYZER~negate_comparison_operator( new_tokens[ tokens_end - 1 ]-lexeme ).
        endif.
      else.
        insert value #( lexeme = 'NOT (' ) into new_tokens index 1.
        insert value #( lexeme = ')' ) into table new_tokens.
    ...
    return ZCC4AZIF_ABAP_ANALYZER~flatten_tokens( new_tokens ).
  endmethod.
  ...
  method _parse_logical_expression.
    if tokens is initial.
      ...
      if ZCC4AZIF_ABAP_ANALYZER~is_logical_connective( tokens[ 1 ] ) = ZCC4AZIF_ABAP_ANALYZER=>logical_connective-not.
        delete tokens index 1.
        return value #(
          ( connective = ZCC4AZIF_ABAP_ANALYZER=>logical_connective-not left = offsets-table + 1
      ...
      while tokens is not initial.
        ...
        if open_brackets = 0.
          data(connective) = ZCC4AZIF_ABAP_ANALYZER~is_logical_connective( <token> ).
          case connective.
            when ZCC4AZIF_ABAP_ANALYZER=>logical_connective-none or
                 ZCC4AZIF_ABAP_ANALYZER=>logical_connective-not.
              case ZCC4AZIF_ABAP_ANALYZER~is_bracket( <token> ).
                when ZCC4AZIF_ABAP_ANALYZER=>bracket_type-opening.
                  open_brackets += 1.
                when ZCC4AZIF_ABAP_ANALYZER=>bracket_type-closing.
                  open_brackets -= 1.
